...
void print_statements(stmt_avec_t *stmt_avec) {
    AVEC_FOREACH(statement, stmt_avec) {
...
static void print_expression(const expr_t *expr, const size_t level) {
    ...
    switch (expr->kind) {
        ...
        case EXPR_ARR_LITERAL: {
            printf("ArrayLiteral (%zu)\n", expr->as.array_literal->size);
            printf("\n");
            AVEC_FOREACH(element, expr->as.array_literal) {
                print_expression(element, level + 1);
            }
            break;
        }
...
static void print_statement(const stmt_t *stmt, const size_t level) {
    ...
    switch (stmt->kind) {
        ...
        case STMT_IF: {
            printf("If (%zu branches)\n", stmt->as._if.then_branches->size);
            size_t i = 0;
            AVEC_FOREACH(branch, stmt->as._if.then_branches) {
                expr_t *condition = expr_avec_at(stmt->as._if.conditions, i);
                indent(level + 1); printf("cond %zu:\n", i);
                print_expression(condition, level + 2);

                indent(level + 1); printf("then %zu:\n", i);
                print_statement(branch, level + 2);
        ...
        case STMT_BLOCK: {
            printf("Block (%zu)\n", stmt->as.block->size);
            AVEC_FOREACH(block, stmt->as.block) {
                print_statement(block, level + 1);
            }
            break;
        }
